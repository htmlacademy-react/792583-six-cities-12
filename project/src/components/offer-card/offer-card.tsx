...
import { AppRoute, Block, BookmarkVersion, OfferCardVersion } from '../../const';
...
export default function OfferCard({ offer, version, onMouseEnter }: OfferScreenProps): JSX.Element {
  const { price, type, title, isPremium, previewImage, isFavorite, id, rating } = offer;
  ...
  return (
    <article onMouseEnter={onMouseEnter && (() => onMouseEnter(id))} className={`${block}__card place-card`}>
      ...
      <div className={`${isFavoriteVersion ? `${block}__card-info` : ''} place-card__info`}>
